...
class UIPrediction:
    ...
    def create_ui(self):
        ...
        # Tạo danh sách model để người dùng chọn
        self.model_options = ["model_1", "model_2", "model_3"]
        self.selected_model = StringVar(self.root)
        self.selected_model.set(self.model_options[0])  # giá trị mặc định

        model_label = Label(loadmodel_panel, text="Select Model:")
        model_label.grid(row=0, column=0, padx=5, pady=5)

        # OptionMenu để chọn model
        model_dropdown = OptionMenu(loadmodel_panel, self.selected_model, *self.model_options)
        model_dropdown.grid(row=0, column=1, padx=5, pady=5)

        # Nút load model
        loadmodel_button = Button(loadmodel_panel, text="6. Load Model", command=self.do_load_model)
        loadmodel_button.grid(row=0, column=2, padx=5, pady=5)
    ...
    def do_save_model(self):
        ...
        save_path = fd.asksaveasfilename(
            ...
            filetypes=filetypes,
            initialdir="Data",  # bạn có thể đổi thành thư mục mặc định khác
        )

        if save_path:  # Nếu người dùng không bấm Cancel
            FileUtil.savemodel(self.lm, save_path)
            messagebox.showinfo("Info", f"Model saved successfully at:\n{save_path}")
        else:
            messagebox.showwarning("Warning", "Save cancelled.")

    def do_load_model(self):
        self.lm = FileUtil.loadmodel("Data/housingmodel.zip")
    ...
    def do_prediction(self):
        ...
        # Tạo DataFrame có tên cột trùng với lúc train
        data = pd.DataFrame([[
        ...
        # Dự đoán
        result = self.lm.predict(data)

        # Gán kết quả ra ô Prediction Price
